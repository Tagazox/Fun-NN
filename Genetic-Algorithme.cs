...
namespace NN
{
    class Genetic_Algorithme
    {
        ...
        public ReseauDeNeurones Best { get; internal set; }
        ...
        public Genetic_Algorithme(int nbE,int nbS, List<ReseauDeNeurones> neur,int nCC,int nbneucc)
        {
            nbEntree = nbE;
            nbSortie = nbS;
            lesreseau = neur;
        ...
        public void CreatioDesnouveauInddividues()
        {
            ...
            for (int i = 0; i < nbIndividuACree; i++)
            {
                ...
                {
                    Couchesortie[i] = new Couches(nbSortie, nbneurperCC);
                }
                else
        ...
        Neurones creationdunenfant(Neurones r1, Neurones r2)
        {
            ...
            for (int i = 0; i < r1.NBin; i++)
            {
                if (rand.NextDouble() < .5f)
                {

                    n.Poids.Add(r1.Poids[i]);
                    n.seuil = r1.seuil;
                }
                else
                {
                    
                        n.Poids.Add(r2.Poids[i]);
                    n.seuil = r2.seuil;
                }
...
**/
        ...
        public ReseauDeNeurones Naissance(ReseauDeNeurones his)
        {

            ReseauDeNeurones mother = his;
            Couches[] Couchecacher = new Couches[ncc];
            Couches Couchesortie ;
            double[] scorerepresentating = new double[lesreseau.Count];
            for (int i = 0; i < lesreseau.Count; i++)
                scorerepresentating[i] = lesreseau[i].score;
            int best = max(scorerepresentating,lesreseau.Count);
            if (Best.score < lesreseau[best].score)
                Best = new ReseauDeNeurones(lesreseau[best]);
            ReseauDeNeurones father = new ReseauDeNeurones( Best);
            if ((rand.NextDouble() < chancetoMutate))
            {
               father= mutatedindividue(nbEntree, nbSortie);

            }
            if (mother.CoucheSortie.someNeurones[0].seuil == father.CoucheSortie.someNeurones[0].seuil)
                father = new ReseauDeNeurones(lesreseau[rand.Next(lesreseau.Count)]);
            for (int j = 0; j < ncc; j++)
            {
                Couchecacher[j] = new Couches(mother.nombreDeneuronesParCoucheCache, nbEntree);
                if (mother.nCC >= 1)
                    for (int k = 0; k < Couchecacher[j].NumOfNeurones; k++)
                        Couchecacher[j].someNeurones[k] = creationdunenfant(mother.Couchecachee[j].someNeurones[k], father.Couchecachee[j].someNeurones[k]);

            }

            if (ncc >= 1)
            {
                Couchesortie = new Couches(nbSortie, nbneurperCC);
            }
            else
                Couchesortie = new Couches(nbSortie, nbEntree);

            for (int j = 0; j < Couchesortie.NumOfNeurones; j++)
                Couchesortie.someNeurones[j] = creationdunenfant(mother.CoucheSortie.someNeurones[j], father.CoucheSortie.someNeurones[j]);

            ReseauDeNeurones retour = new ReseauDeNeurones(mother);
              for (int j = 0; j < ncc; j++)
                retour.Couchecachee[j] =new Couches( Couchecacher[j]);
            retour.CoucheSortie = new Couches(Couchesortie);
            lesreseau.Add(retour);
            System.Console.WriteLine("BestScore: "+ Best.score.ToString());
            return retour;
        }
    }
}
